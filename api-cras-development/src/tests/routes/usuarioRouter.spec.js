...
describe ('/GET em usuarios', () => {
    it("Deve retornar uma lista de usuários", async () => {
        ...
        expect(dados._body.docs[2].nome).toContain('Eloisa Silva');
    });
});
...
describe ('/POST em usuarios', () => {
    it.skip("Deve cadastrar um usuário", async () => {
        ...
        .send({           
            nome: 'Priscila',
            email: "priscila.teodoro@hotmail.com",
            senha: "123"
        })
        .expect(201);
...
describe ('/PACTH/ID em usuários', () => {
    it("Deve atualizar usuário cadastrado", async () => {
        const idUsuarioPatch = "641519d864d27f867d3488d5"; 
        const dados = await request(app)
        .patch(`/usuarios/${idUsuarioPatch}`)
        .set('Authorization', `Bearer ${token}`)
        .send({email: "pri.teodoro@hotmail.com"})
        .expect(200);
